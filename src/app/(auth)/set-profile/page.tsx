...
const SetProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      // try {
      //   const getUser = await callApi().post(
      //     userRoute,
      //     { tokenKey: "setProfile" },
      //     {
      //       headers: {
  ...
  const submitHandler = async (e: any) => {
    ...
    try {
      ...
      if (err.message === "No image") {
        ...
        //     setProfileRoute,
        //     { id: user._id },
        //     { headers: { Authorization: cookies["set-profile-token"] } }
        //   );
        //   if (res.status === 200) {
        //     removeCookies("set-profile-token");
        //     setCookie(res.data.token);
    ...
    >
      <div className="w-5/6 md:w-1/2 xl:w-2/3">
        <div className="flex flex-col">
          {errors.map((err: any) => (
            <span key={err} className="text-red-600 text-md">
              {err}
            </span>
          ))}
        </div>
            ...
            >
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                ...
                <p className="text-xs text-gray-500">
                  JPEG, PNG, JPG (Size should be less than 5 MB)
                </p>
              </div>
              <input
                id="profile"
